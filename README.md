# 👋 Hello, Alex's here!

🎓 CS student at IUT d'Aix-Marseille 

🎮 8+ years messing around with code and music

## 😍 My favorite projects 

1. [**Custom Assembly compiler/interpreter in C**](https://github.com/Alexandre-SBEGHEN/RAM-Machine-ASM-Compiler-Interpreter-V2) — *solo*
2. [**The Brainrot City website**](https://brainrotcity.alwaysdata.net/) — *team project*
3. [**Candy Crush Clone in C++**](https://github.com/Alexandre-SBEGHEN/BUT-SAE1.01-Candy-Crush) — *team project*

## 🧐 Stuff I would love to create

- A [Brainfuck](https://en.wikipedia.org/wiki/Brainfuck) interpreter in Assembly
- A real-time music sequencer for videogames
